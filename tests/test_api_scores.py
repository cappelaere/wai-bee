...
def test_get_top_scores_default(test_client):
    """Test getting top scores with default limit."""
    response = test_client.get("/top_scores")
...
def test_get_top_scores_custom_limit(test_client):
    """Test getting top scores with custom limit."""
    limit = 5
    response = test_client.get(f"/top_scores?limit={limit}")
...
def test_get_top_scores_validates_limit(test_client):
    """Test that limit parameter is validated."""
    # Test limit too high
    response = test_client.get("/top_scores?limit=200")
    assert response.status_code == 422  # Validation error
    
    # Test limit too low
    response = test_client.get("/top_scores?limit=0")
    assert response.status_code == 422  # Validation error


def test_top_scores_structure(test_client):
    ...
    if data["top_scores"]:
        ...
        assert 0 <= score["completeness_score"] <= 30
        assert 0 <= score["validity_score"] <= 30
        assert 0 <= score["attachment_score"] <= 40


def test_get_individual_score_valid(test_client, sample_wai_number):
    """Test getting score for a valid WAI number."""
    response = test_client.get(f"/score?wai_number={sample_wai_number}")
    ...
    assert 0 <= data["completeness_score"] <= 30
    assert 0 <= data["validity_score"] <= 30
    assert 0 <= data["attachment_score"] <= 40


def test_get_individual_score_invalid(test_client, invalid_wai_number):
    """Test getting score for an invalid WAI number."""
    response = test_client.get(f"/score/{invalid_wai_number}")
...
def test_top_scores_sorted_descending(test_client):
    """Test that top scores are sorted in descending order."""
    response = test_client.get("/top_scores?limit=10")
